...
def closeNotifyModal(browser):
    try:
        modal_btn = browser.find_element_by_xpath('//div[@aria-haspopup="false" and @role="button" and @data-focusable="true" and @tabindex="0"]')
        modal_btn_child = modal_btn.find_element_by_xpath('./div[@dir="auto"]')
        modal_btn.click()
...
def makeTweetScreenshot(browser, url):
    ...
    try:
        ...
        wait(browser, 10).until(EC.presence_of_element_located(
            (By.XPATH, '//article[@data-focusable="true" and @role="article"]')))
        ...
        while repeatCount < 2:
            try:
                ...
                articles = primary_column.find_elements_by_xpath('.//article[@data-focusable="true" and @role="article"]')
                break
