...
class MSEMasterFile:
    ...
    def load(self):
        ...
        if os.path.isfile('EMASTER'):
            ...
            self.records_count = struct.unpack("H", self.file_handle.read(2))[0] # czy moze 1 bajt i B?
    ...
    def save_symbols(self, save_path, symbols = []):
        with open(save_path, 'wb') as writer:
            symbol_count = len(symbols)
            buf = bytearray(192)
            temp_buf = struct.pack("H", symbol_count)
            # symbol count and last file
            buf[0:2] = temp_buf
            buf[2:4] = temp_buf
            writer.write(buf)
            ...
            for i in range(symbol_count):
                ...
                #     break
                buf = bytearray(192)
                buf[0:2] = bytearray(b'\x36\x36')
                # file number
                # buf[2] = struct.pack("B", i + 1)[0]
                buf[2] = struct.pack("B", (i + 1) % 256)[0]
                # fields
                buf[6:10] = bytearray(b'\x06\x3f\x00\x20')
                # stock symbol code
                buf[11:25] = symbols[i]['code'].encode(self.encoding)
                # stock symbol name
                buf[32:48] = symbols[i]['name'].encode(self.encoding)
                # time frame, first and end date
                buf[60:80] = bytearray(b'\x44\x00\x00\x00\x70\x2A\x74\x49\x00\x00\x00\x00\x30\x7a\x6d\x49\x00\x00\x00\x00')
...
class MetastockFiles:
    def __init__(self, encoding, precision=None):
        ...
        master.close()
        emaster.load()
        if emaster.records_count > 0:
            for i in range(emaster.records_count):
